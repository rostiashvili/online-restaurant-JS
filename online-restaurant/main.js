...
function filter(products){
    var idk = products.filter(product => product.categoryId == filteredId)
    createCards(idk)
}
...
function final(products){
    if(sliderValue != -1){
    if(filteredId != null){
        var advancedFiltered = products.filter(product => product.spiciness == sliderValue && product.nuts == checkboxNuts.checked && product.vegeterian == checkboxVeg.checked && product.categoryId == filteredId)
    }else{
    var advancedFiltered = products.filter(product => product.spiciness == sliderValue && product.nuts == checkboxNuts.checked && product.vegeterian == checkboxVeg.checked)
    }
    
    createCards(advancedFiltered)
